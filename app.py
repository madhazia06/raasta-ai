...
from ranking import get_ranked_routes, get_ranked_routes_by_name
...
def _google_maps_key():
    """Read the Google key from Streamlit secrets first, then environment."""
    try:
...
def resolve_place(place: str):
    ...
    if stop is not None and stop.lat is not None and stop.lon is not None:
        return {
            ...
            "lat": stop.lat,
            "lon": stop.lon,
            "provider": "RAASTA stop dataset",
            "is_exact_stop": True,
        }

    result = geocode_place(place, _google_maps_key())
    if result:
        result["query"] = place
...
def get_real_route(start_place: dict, destination_place: dict, selected_preferences: set):
    ...
    start_walk = walking_route(start_place, route["start_stop"], key)
    end_walk = walking_route(route["end_stop"], destination_place, key)

    # Replace the engine's straight-line walking estimate with Google walking
    # duration when available (or the fallback walking estimate otherwise).
...
def show_route_results():
    ...
    if not start_place.get("is_exact_stop"):
        ...
        st.info(
            f"📍 **{start} is not a stop in the RAASTA dataset.** "
    ...
    if variant["start_walk"]["provider"] == "estimated" or variant["end_walk"]["provider"] == "estimated":
        st.caption("Walking route is estimated. Add GOOGLE_MAPS_API_KEY to enable Google walking directions.")
