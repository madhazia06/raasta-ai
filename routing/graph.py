...
class Stop:
    def __init__(self, stop_id, name, lat, lon, num_routes=1, served_by=""):
        self.stop_id = stop_id
        self.name = name
        self.lat = float(lat) if lat not in (None, "") else None
        self.lon = float(lon) if lon not in (None, "") else None
        # How many routes serve this stop -- used to prefer major/hub stops
        # when a user's typed name is ambiguous (e.g. "Shahdara" could mean
        # any of 3 different real stops with that word in the name).
        self.num_routes = int(num_routes) if num_routes not in (None, "") else 1
        self.served_by = served_by.split("|") if served_by else []
...
class TransportGraph:
    ...
    def load_stops(self, stops_csv_path):
        with open(stops_csv_path, newline="", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                stop = Stop(
                    row["stop_id"], row["stop_name"], row.get("latitude"), row.get("longitude"),
                    num_routes=row.get("num_routes", 1), served_by=row.get("served_by", ""),
                )
    ...
    def find_stop_by_name(self, name):
        ...
        a user's question). Tries, in order:
          ...
          2. Prefix match: the stop's name STARTS WITH the query (e.g.
             typing "Shahdara" should prefer "Shahdara Metro Bus Station"
             over "Kalma Chowk" even though neither is an exact match --
             but among several "Shahdara ..." stops, this alone doesn't
             pick a winner, so see the tie-break below)
          3. General substring match (query appears anywhere in the name,
             or vice versa) as a looser fallback
        Returns the matching Stop, or None if nothing reasonable was found.

        TIE-BREAKING when multiple stops match equally well: prefers the
        stop served by the MOST routes (a bigger interchange/hub is more
        likely what a person means by a short, generic place name), and
        only falls back to "shortest name" if that's still tied.
        """
        ...
        for stop_name_norm, stop_id in self.name_index.items():
            if stop_name_norm.startswith(key) or key.startswith(stop_name_norm):
                prefix_candidates.append(self.stops[stop_id])
            elif key in stop_name_norm or stop_name_norm in key:
                substring_candidates.append(self.stops[stop_id])

        candidates = prefix_candidates or substring_candidates
        if not candidates:
            return None

        # Tie-break priority, in order:
        #   1. Served by more routes (bigger interchange/hub)
        #   2. Served by Metro (the flagship system -- its stops tend to be
        #      the most recognizable landmark names, e.g. a city terminus)
        #   3. Shortest name (usually the more "canonical" match)
        bus_type_priority = {"Metro": 2, "Electro": 1, "Speedo": 0}

        def score(s):
            best_bus_type_score = max((bus_type_priority.get(bt, 0) for bt in s.served_by), default=0)
            return (s.num_routes, best_bus_type_score, -len(s.name))
